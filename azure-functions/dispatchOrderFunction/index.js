...
module.exports = async function (context, req) {
  try {
    ...
    const { dispatch_order_ref, crate_items, destination_warehouse } = requestBody;
    if (!dispatch_order_ref) {
      context.res = {
        status: 400,
        headers: corsHeaders,
        body: { success: false, error: 'Dispatch order reference is required' }
      };
      return;
    }
    ...
    // ...rest of your logic, call createDispatchOrder and return response...
    context.res = {
      status: 200,
      headers: corsHeaders,
      body: { success: true, message: 'Dispatch order created (implement logic)' }
    };
    // ------------------------
