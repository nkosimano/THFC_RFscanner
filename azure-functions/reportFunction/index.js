  ...
  try {
    ...
    if (!start_date) {
      context.res = {
        status: 400,
        headers: corsHeaders,
        body: { success: false, error: 'Start date is required' }
      };
      return;
    }
    if (!end_date) {
      context.res = {
        status: 400,
        headers: corsHeaders,
        body: { success: false, error: 'End date is required' }
      };
      return;
    }
    ...
    context.res = {
      status: 200,
      headers: corsHeaders,
      body: { success: true, message: 'Donation report generated (implement logic)' }
    };
    // ------------------------
