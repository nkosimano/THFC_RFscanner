...
module.exports = async function (context, req) {
  try {
    ...
    const { production_quantity, requested_date, notes } = requestBody;
    if (!production_quantity || production_quantity <= 0) {
      context.res = {
        status: 400,
        headers: corsHeaders,
        body: { success: false, error: 'Valid production quantity is required' }
      };
      return;
    }
    ...
    // ...rest of your logic, call makeZohoInventoryRequest and return response...
    context.res = {
      status: 200,
      headers: corsHeaders,
      body: { success: true, message: 'Production order created (implement logic)' }
    };
    // ------------------------
